...
export async function sendChat(messages: ChatMessage[], signal?: AbortSignal): Promise<ChatMessage> {
  ...
  if (!response.ok) throw new Error(body && "detail" in body ? body.detail ?? "The assistant is unavailable." : "The assistant is unavailable.");
  if (!body || !("message" in body)) throw new Error("The assistant returned an invalid response.");
  return { ...body.message, id: crypto.randomUUID() };
}

export async function sendDocumentChat(question: string, documentId: string, signal?: AbortSignal): Promise<ChatMessage> {
  const response = await fetch("/api/documents/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question, document_id: documentId }), signal });
  const body = await response.json().catch(() => null) as DocumentChatResponse | { detail?: string } | null;
  if (!response.ok) throw new Error(body && "detail" in body ? body.detail ?? "Document Q&A is unavailable." : "Document Q&A is unavailable.");
